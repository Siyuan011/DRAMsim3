...
class Generator():
    ...
    def __init__(self, stream_type, interarrival, ratio, gb, bandwidth):
    ...
    def _stream_gen(self):
        op = self._get_op()
        if op == 'r':
            self._last_rd_addr += self.bandwidth
            return (op, self._last_rd_addr)
        else:
            self._last_wr_addr += self.bandwidth
...
if __name__ == '__main__':
    ...
    parser.add_argument('-b', '--bandwidth', type=int, default=64,
                        help='bandwidth')
    parser.add_argument('-m', '--output-file-name', default="check_file_name",
    ...
    g = Generator(stream_type, args.interarrival, args.ratio, args.gb, args.bandwidth)
